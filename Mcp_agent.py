...
def run():
    ...
    def process_files(state: ApplicationState) -> ApplicationState:
        ...
        try:
            # PDF Processing
            if pdf_files := state['processed_files'].get('pdf'):
                reader = PdfReader(pdf_file)
                processed['pdf_text'] = " ".join([page.extract_text() for page in reader.pages])[:500] + "..."
            
            # Image Verification
            if img_files := state['processed_files'].get('image'):
                Image.open(img_files).verify()
                processed['id_verified'] = True
                
            # Audio Processing
            if audio_files := state['processed_files'].get('audio'):
    ...
    with st.sidebar:
        ...
        select_applicant = st.selectbox("👤 Select Applicant", df['name'].unique())
        pdf_files = st.file_uploader("📑 Supporting Documents (PDF)", type=["pdf"])
        img_files = st.file_uploader("🖼️ ID Verification Image", type=["jpg", "jpeg", "png"])
        audio_files = st.file_uploader("🔊 Voice Explained", type=["mp3", "wav"])
        analyze_button = st.button("🚀 Assess Application")

    if analyze_button:
        # Initialize state with model context
        initial_state = {
            "model_context": model_config,
            "applicant_data": df[df['name'] == select_applicant].iloc[0].to_dict(),
            "processed_files": {
                "pdf": pdf_files,
                "image": img_files,
                "audio": audio_files
        ...
        with col1:
            ...
            **Dependents:** {applicant['dependents']}
            ...
            if img_files:
                st.image(Image.open(img_files), width=250)
